...
<script type="text/html" id="newsListBar">
    <a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="rolePermission">分配权限</a>
    <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
    <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="del">删除</a>
</script>
<script>
    var roleIndexUrl ="{{route('role.index')}}",//列表数据
        roleChangeStatusUrl="{{route('role.status')}}",
        roleDelUrl = "{{route('role.roleDel')}}",
        roleAddUrl = "{{route('role.add')}}",
        roleSaveUrl = "{{route('role.save')}}",
        rolePermissionUrl = "{{route('role.rolePermission')}}"

</script>
@include('zng::common.footer')
<script type="text/javascript" src="{{asset('zng/assets/js/admin/role.js')}}"></script>
