...
<script>
    var roleIndexUrl ="{{route('user.index')}}",//列表数据
        roleChangeStatusUrl="{{route('user.status')}}",
        roleDelUrl = "{{route('user.del')}}",
        roleAddUrl = "{{route('user.add')}}",
        roleSaveUrl = "{{route('user.save')}}"
        roleDelAllUrl = "{{route('user.delAll')}}"
</script>
@include('zng::common.footer')
<script type="text/javascript" src="{{asset('zng/assets/js/admin/user.js')}}"></script>
